...
(defn distance-between [point1 point2]
 (let [R 6372.8 ; kilometers
       dlat (Math/toRadians (- (Float. (point2 0)) (Float. (point1 0))))
       dlon (Math/toRadians (- (Float. (point2 1)) (Float. (point1 1))))
       lat1 (Math/toRadians (Float. (point1 0)))
       lat2 (Math/toRadians (Float. (point2 0)))
       a (+ (* (Math/sin (/ dlat 2)) (Math/sin (/ dlat 2))) (* (Math/sin (/ dlon 2)) (Math/sin (/ dlon 2)) (Math/cos lat1) (Math/cos lat2)))]
   (* R 2 (Math/asin (Math/sqrt a)))))

(defn bearing-to [point1 point2]

  (let [[lat1 long1] (deg2rad (mapv #(Float. %) point1))
        [lat2 long2] (deg2rad (mapv #(Float. %) point2))
